...
if sys.platform == "win32":
    ...
    def _twain1_alloc(size: int) -> ct.c_void_p:
        return check_windows.GlobalAlloc(check_windows.GMEM_ZEROINIT, size)

    _twain1_free = check_windows.GlobalFree
    _twain1_lock = check_windows.GlobalLock
    _twain1_unlock = check_windows.GlobalUnlock
    ...
    class _Image(_IImage):
        ...
        def save(self, filepath: str=None):
            ...
            return check_windows.dib_to_bm_file(self._handle, filepath)  # type: ignore # needs fixing
else:
    # Mac
    def _twain1_alloc(size: int) -> ct.c_void_p:
...
class Source:
    ...
    def _modal_loop(self, event_callback: collections.abc.Callable[[int], None] | None) -> None:
        ...
        while not done:
            ...
            #if not check_windows.GetMessage(ct.byref(msg), 0, 0, 0):  # type: ignore # needs fixing
            ...
            if rc == constants.TWRC_NOTDSEVENT:
                ...
                # Windows mechanisms
                check_windows.TranslateMessage(ct.byref(msg))  # type: ignore # needs fixing
                check_windows.DispatchMessage(ct.byref(msg))  # type: ignore # needs fixing
    ...
    def acquire_file(
        ...
        after: collections.abc.Callable[[int], ] = lambda more: None,
        show_ui: bool = True,
        modal: bool = False,
        idPai: str = ""
    ) -> None:
    ...
    def acquire_natively(
        ...
        def xfer_ready_callback() -> int:
            ...
            after(_Image(handle), more, idPai)  # type: ignore # needs fixing
            return more
...
from .check_windows import *  # noqa
